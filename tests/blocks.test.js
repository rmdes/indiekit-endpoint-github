...
// Phase 7c — github v2 `get blocks()`. Assertions replicate the invariants of
...
test("GITHUB_BLOCKS: exactly the github-repos widget", () => {
  assert.deepEqual(GITHUB_BLOCKS.map((b) => b.id), ["github-repos"]);
});

test("GITHUB_BLOCKS: every entry satisfies the v2 block contract", () => {
  for (const b of GITHUB_BLOCKS) {
...
test("GITHUB_BLOCKS: github-repos is a config-less homepage sidebar widget, api-sourced, bespoke", () => {
  const [b] = GITHUB_BLOCKS;
  ...
  assert.ok(!b.render, "bespoke — theme owns widgets/github-repos.njk + js/widgets/github-repos.js");
});
