...
export function extractContributions(events) {
  ...
  return events
    ...
    // The public events feed carries a minimal pull_request object — id,
    // number, url, base, head — with no title and no html_url. An entry built
    // from it renders as a bare "repo #944" with a dead link. Dropping those
    // lets every caller's "no contributions" branch reach the Search API,
    // which returns the real titles and is not capped to the events window.
    .filter((contribution) => contribution.title && contribution.url);
}
